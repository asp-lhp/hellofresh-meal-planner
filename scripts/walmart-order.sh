#!/bin/bash
# Walmart Grocery Order Automation via OpenClaw
# Usage: ./walmart-order.sh <meal_plan_id>

set -e

MEAL_PLAN_ID=${1:-1}
EXPORT_FILE="/tmp/walmart-order-${MEAL_PLAN_ID}.json"

echo "🛒 Walmart Grocery Order Automation"
echo "===================================="
echo ""
...
# Step 4: Launch OpenClaw with Walmart ordering prompt
# Combine the instructions with the JSON data
PROMPT_MESSAGE="I need you to order groceries from Walmart.com for delivery. Here's what to do:

IMPORTANT: Use browser automation to complete this entire workflow.

## Step 1: Navigate to Walmart.com
1. Go to https://www.walmart.com/grocery
2. If not logged in, pause and let me log in manually
3. Set delivery address (should be saved if logged in)

## Step 2: Add Items to Cart
For each item in the shopping list:
1. Search for the item using the searchTerm field
2. Select the best match (prefer exact brand/quantity when available)
3. Add the specified quantity to cart
4. If item not found, note it for manual review

## Step 3: Review & Handle Substitutions
1. Review all items in cart
2. For each item, check Walmart substitution suggestions
...
openclaw agent --local --session-id "walmart-$(date +%Y%m%d-%H%M%S)" --message "$PROMPT_MESSAGE"
